from flask import Flask, render_template_string, request, redirect, session, jsonify, after_this_request
import config
import os
...
<head>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .box h2 { text-align: center; margin-bottom: 25px; font-size: 24px; color: #00bfff; }
        .box input {
            width: 100%;
            padding: 12px 16px;
            margin: 10px 0 20px;
        ...
        .box input:focus { background: #1f3460; box-shadow: 0 0 0 2px #00bfff; }
        .box button {
            width: 100%;
            padding: 12px;
            background: #00bfff;
...
<head>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        body { font-family: 'Segoe UI', Arial, sans-serif; background: #0d0d1a; color: white; padding: 16px; min-height: 100vh; }
        .container { max-width: 1000px; margin: 0 auto; background: #16213e; padding: 20px; border-radius: 16px; border: 1px solid #1f3a6e; }
        h1 { font-size: 24px; color: #00bfff; text-align: center; margin-bottom: 20px; }
        .status-bar { display: flex; flex-wrap: wrap; justify-content: space-between; background: #0f2040; padding: 14px 18px; border-radius: 10px; margin-bottom: 20px; gap: 10px; font-size: 14px; }
        .status-bar span { background: #1a2a4a; padding: 4px 12px; border-radius: 20px; }
        .section { background: #0f2040; padding: 16px; border-radius: 10px; margin-bottom: 20px; }
        .section h3 { font-size: 18px; margin-bottom: 12px; color: #00bfff; }
        .input-group { display: flex; flex-wrap: wrap; gap: 10px; margin: 10px 0; }
        .input-group input { flex: 1; min-width: 200px; padding: 10px 14px; border-radius: 8px; border: none; background: #1a2a4a; color: white; font-size: 14px; outline: none; }
        .input-group input:focus { background: #1f3460; box-shadow: 0 0 0 2px #00bfff; }
        .btn { padding: 10px 20px; border: none; border-radius: 8px; font-weight: 600; cursor: pointer; transition: 0.3s; font-size: 14px; }
        ...
        th, td { padding: 10px 8px; text-align: left; border-bottom: 1px solid #1f3a6e; }
        ...
        .footer { text-align: center; color: #666; font-size: 13px; margin-top: 20px; padding-top: 15px; border-top: 1px solid #1f3a6e; display: flex; justify-content: space-between; flex-wrap: wrap; gap: 10px; }
        ...
        .toast { position: fixed; bottom: 20px; right: 20px; background: #00ff7f; color: #0d0d1a; padding: 12px 24px; border-radius: 10px; font-weight: 600; box-shadow: 0 4px 20px rgba(0,255,127,0.3); opacity: 0; transform: translateY(20px); transition: 0.4s ease; pointer-events: none; z-index: 999; max-width: 90%; }
...
<body>
    <div class="container">
        ...
        <div class="section">
            ...
            <div style="overflow-x: auto;">
                <table id="tokensTable">
                    <tr><th>#</th><th>الحساب</th><th>الحالة</th><th>الإجراء</th></tr>
                    {% for t in tokens %}
                    <tr>
                        <td>{{ loop.index }}</td>
                        ...
                        <td><button onclick="deleteToken('{{ t.token }}')" class="btn btn-danger">🗑️</button></td>
                    </tr>
                    {% endfor %}
                </table>
            </div>
        ...
        <div class="section">
            ...
            <div class="webhook-status">
                📌 ويب هوك الإشعارات: <span class="{% if webhook_url %}ok{% else %}no{% endif %}">
                    {% if webhook_url %} ✅ مضبوط {% else %} ❌ غير مضبوط {% endif %}
                </span>
                &nbsp;|&nbsp;
                📋 ويب هوك السجلات: <span class="{% if log_webhook_url %}ok{% else %}no{% endif %}">
    ...
    <script>
        ...
        async function refreshData() {
            ...
            try {
                const resp = await fetch('/api/dashboard_data', { cache: 'no-cache', headers: { 'Cache-Control': 'no-cache' } });
                ...
                data.tokens.forEach((t, i) => {
                    const cls = t.status === 'online' ? 'online' : 'offline';
                    const txt = t.status === 'online' ? '🟢 شغال' : '🔴 معطل';
                    const name = t.username || t.token.substring(0,20) + '...';
                    html += `<tr><td>${i+1}</td><td><strong>${name}</strong></td><td class="${cls}">${txt}</td><td><button onclick="deleteToken('${t.token}')" class="btn btn-danger">🗑️</button></td></tr>`;
                });
        ...
        async function addToken(e) {
            ...
            const resp = await fetch('/add_token', { method: 'POST', body: formData, cache: 'no-cache', headers: { 'Cache-Control': 'no-cache' } });
            if (resp.ok) {
        ...
        async function deleteToken(token) {
            ...
            const resp = await fetch('/delete_token', { method: 'POST', body: formData, cache: 'no-cache', headers: { 'Cache-Control': 'no-cache' } });
            if (resp.ok) {
        ...
        async function updateWebhooks(e) {
            e.preventDefault();
            const formData = new FormData();
            formData.append('webhook_url', document.getElementById('webhookUrl').value.trim());
            formData.append('log_webhook_url', document.getElementById('logWebhookUrl').value.trim());
            const resp = await fetch('/update_webhooks', { method: 'POST', body: formData, cache: 'no-cache', headers: { 'Cache-Control': 'no-cache' } });
            if (resp.ok) {
...
def setup_dashboard(app):
    ...
    # ===== تعطيل الكاش في جميع الاستجابات =====
    @app.after_request
    ...
    def add_token():
        ...
        if new_token:
            ...
            if new_token not in tokens:
                ...
                log_info(f"➕ تم إضافة توكن: {new_token[:15]}...")
        return '', 200
